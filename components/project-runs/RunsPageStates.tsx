'use client'

import { RUN_ROW_GRID_CLASS } from '@/components/project-runs/RunRow'
import { Button } from '@/components/ui/Button'

type EmptyMode = 'empty' | 'search' | 'running' | 'failed' | 'done' | 'filtered'

interface RunsPageEmptyStateProps {
  mode: EmptyMode
  search: string
  activeFilterLabel: string
  totalEntries: number
  runningCount: number
  failedCount: number
  projectScopeLabel: string
  onClearView: () => void
  onResetScope?: () => void
}

function LoadingRow({ compact = false }: { compact?: boolean }) {
  return (
    <div className="border-b border-border last:border-b-0 lg:col-span-full lg:grid lg:grid-cols-subgrid lg:gap-x-3 lg:items-center">
      <div className={`grid gap-3 px-4 py-2.5 lg:contents ${RUN_ROW_GRID_CLASS} lg:items-center`} style={{ opacity: compact ? 0.68 : 1 }}>
        <div className="flex min-w-0 items-start gap-2">
          <div className="skeleton mt-0.5 h-6 w-6 shrink-0 rounded" />
          <div className="skeleton mt-0.5 h-5 w-5 shrink-0 rounded-full" />
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 items-center gap-2">
              <div className="skeleton h-5 w-16 shrink-0 rounded" />
              <div className="skeleton h-4 w-48 max-w-[75%] rounded" />
            </div>
            <div className="mt-1 flex flex-wrap items-center gap-2">
              <div className="skeleton h-3 w-[5rem] rounded" />
              <div className="skeleton h-3 w-20 rounded" />
              <div className="skeleton h-3 w-24 rounded" />
            </div>
          </div>
        </div>

        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-1.5">
            <div className="skeleton h-5 w-20 rounded-full" />
            <div className="skeleton h-5 w-24 rounded-full" />
            {!compact && <div className="skeleton h-5 w-[4.5rem] rounded-full" />}
          </div>
          <div className="mt-1.5 flex items-center gap-2">
            <div className="skeleton h-3 w-12 rounded" />
            <div className="skeleton h-3 w-52 max-w-[80%] rounded" />
          </div>
        </div>

        <div className="flex items-center gap-2 lg:justify-end">
          <div className="skeleton h-4 w-14 rounded" />
        </div>

        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            <div className="skeleton h-3 w-24 rounded" />
            <div className="skeleton h-3 w-[4.5rem] rounded" />
          </div>
        </div>

        <div className="flex items-center justify-start lg:justify-end">
          <div className="skeleton h-7 w-[4.5rem] rounded-md" />
        </div>
      </div>
    </div>
  )
}

export function RunsPageLoadingState() {
  return (
    <div className="flex flex-col gap-4" aria-label="Loading runs">
      <div className="rounded-lg border border-border bg-bg-secondary">
        <div className="grid gap-3 border-b border-border p-3 lg:grid-cols-[minmax(300px,1fr)_auto] lg:items-start">
          <div>
            <div className="skeleton h-7 w-28 rounded" />
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <div className="skeleton h-3 w-40 rounded" />
              <div className="skeleton h-3 w-28 rounded" />
              <div className="skeleton h-3 w-24 rounded" />
            </div>
          </div>

          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            <div className="skeleton h-8 w-20 rounded-md" />
            <div className="skeleton h-8 w-40 rounded-md" />
            <div className="skeleton h-8 w-64 rounded-md" />
          </div>
        </div>

        <div className="grid gap-2 border-b border-border p-3 sm:grid-cols-2 xl:grid-cols-4">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="rounded-md border border-border bg-bg-primary px-3 py-2">
              <div className="skeleton h-3 w-[4.5rem] rounded" />
              <div className="mt-2 skeleton h-5 w-20 rounded" />
            </div>
          ))}
        </div>

        <div className="flex items-center gap-1.5 overflow-hidden p-1">
          {Array.from({ length: 8 }).map((_, index) => (
            <div
              key={index}
              className="skeleton h-7 rounded-full"
              style={{ width: `${index < 4 ? 72 : 82}px`, opacity: 1 - index * 0.08 }}
            />
          ))}
        </div>
      </div>

      {[
        { label: 'today', rows: 4 },
        { label: 'yesterday', rows: 3 },
      ].map((group, groupIndex) => (
        <div key={group.label}>
          <div className="mb-1.5 flex items-center gap-2 px-1">
            <div className="skeleton h-3 w-20 rounded" />
            <div className="skeleton h-3 w-6 rounded" />
            <div className="h-px flex-1 bg-border/60" />
          </div>

          <div className={`rounded-lg border border-border bg-bg-primary lg:grid ${RUN_ROW_GRID_CLASS} lg:gap-x-3`}>
            <div className="hidden border-b border-border bg-bg-secondary pl-4 pr-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-text-tertiary lg:col-span-full lg:grid lg:grid-cols-subgrid lg:gap-x-3">
              <span>wanted</span>
              <span>done / progress</span>
              <span className="text-right">duration</span>
              <span className="text-right">usage</span>
              <span className="text-right">actions</span>
            </div>

            {Array.from({ length: group.rows }).map((_, rowIndex) => (
              <LoadingRow key={`${group.label}:${rowIndex}`} compact={groupIndex === 1 && rowIndex > 0} />
            ))}
          </div>
        </div>
      ))}
    </div>
  )
}

export function RunsPageEmptyState({
  mode,
  search,
  activeFilterLabel,
  totalEntries,
  runningCount,
  failedCount,
  projectScopeLabel,
  onClearView,
  onResetScope,
}: RunsPageEmptyStateProps) {
  const queryValue = search.trim()

  const title = (() => {
    switch (mode) {
      case 'empty':
        return projectScopeLabel === 'all projects' ? 'No runs yet' : 'No runs for this project yet'
      case 'search':
        return 'No runs match this search'
      case 'running':
        return 'Nothing is running right now'
      case 'failed':
        return 'No runs need attention'
      case 'done':
        return 'No completed runs in view'
      default:
        return `No ${activeFilterLabel.toLowerCase()} runs in view`
    }
  })()

  const detail = (() => {
    switch (mode) {
      case 'empty':
        return projectScopeLabel === 'all projects'
          ? 'Runs appear here after terminal work, agent runs, or release pipeline activity starts.'
          : `The current scope is ${projectScopeLabel}. Switch back to all projects to compare other activity.`
      case 'search':
        return `Nothing in ${activeFilterLabel.toLowerCase()} matches “${queryValue}”.`
      case 'running':
        return `There is no active run in ${projectScopeLabel} right now.`
      case 'failed':
        return 'Visible runs are either done cleanly or still in progress.'
      case 'done':
        return 'The current view only contains running or attention-needed work.'
      default:
        return 'This filter is empty for the current history window.'
    }
  })()

  return (
    <div className="rounded-lg border border-border bg-bg-secondary">
      <div className="border-b border-border px-4 py-3">
        <div className="text-sm font-medium text-text-primary">{title}</div>
        <div className="mt-1 text-xs text-text-secondary">{detail}</div>
      </div>

      <div className="grid gap-2 px-4 py-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-md border border-border bg-bg-primary px-3 py-2">
          <div className="text-[10px] uppercase tracking-wider text-text-tertiary">scope</div>
          <div className="mt-1 truncate font-mono text-sm font-semibold text-text-primary" title={projectScopeLabel}>
            {projectScopeLabel}
          </div>
        </div>
        <div className="rounded-md border border-border bg-bg-primary px-3 py-2">
          <div className="text-[10px] uppercase tracking-wider text-text-tertiary">entries</div>
          <div className="mt-1 font-mono text-sm font-semibold tabular-nums text-text-primary">{totalEntries}</div>
        </div>
        <div className="rounded-md border border-border bg-bg-primary px-3 py-2">
          <div className="text-[10px] uppercase tracking-wider text-text-tertiary">running</div>
          <div className="mt-1 font-mono text-sm font-semibold tabular-nums text-status-info">{runningCount}</div>
        </div>
        <div className="rounded-md border border-border bg-bg-primary px-3 py-2">
          <div className="text-[10px] uppercase tracking-wider text-text-tertiary">{queryValue ? 'query' : 'needs attention'}</div>
          <div
            className={`mt-1 truncate font-mono text-sm font-semibold ${queryValue ? 'text-text-primary' : 'tabular-nums text-status-error'}`}
            title={queryValue || String(failedCount)}
          >
            {queryValue || failedCount}
          </div>
        </div>
      </div>

      <div className="flex flex-wrap items-center gap-2 border-t border-border px-4 py-3">
        {(mode !== 'empty' || queryValue || activeFilterLabel !== 'all') && (
          <Button type="button" size="sm" variant="secondary" onClick={onClearView}>
            Clear filters
          </Button>
        )}
        {onResetScope && (
          <Button type="button" size="sm" variant="primary" onClick={onResetScope}>
            Show all projects
          </Button>
        )}
      </div>
    </div>
  )
}
