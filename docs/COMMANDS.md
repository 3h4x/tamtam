...
- `pnpm build` — production build through `scripts/build-with-metrics.mjs`. The wrapper now defaults to `next build --webpack`, prints CPU/RSS plus host RAM/swap/paging metrics, and writes the build output to the live `.next` unless `TAMTAM_DIST_DIR` overrides it. Runs `prebuild` first, which regenerates `public/workflow-graph.svg` from `lib/workflows/pipeline-spec.ts` when Chrome/Chromium is available. If no browser is available but the committed SVG already exists, the generator keeps the existing SVG and lets the build continue. Runs `postbuild` afterward only for the live `.next` build, scanning route `.nft.json` files under `.next/server` and failing when any route traces an oversized file list; see `docs/PROFILING.md` for Turbopack/NFT troubleshooting. Wrapper override: `TAMTAM_BUILD_METRICS_OFF=1`. Do not pipe this command through `tail`/`head`/`grep`; it already streams progress.
- `pnpm build:turbopack` — run the same metrics wrapper against Next 16's default turbopack build for comparison runs.
- `pnpm build:webpack-test` — isolated webpack comparison build into `.next-webpack-test` so the live `.next` stays untouched.
