...
- `pnpm run rebuild` — graceful build + PM2 restart via `scripts/rebuild-safe.sh`: pause new jobs, wait for active pipeline/agent/run jobs to drain, stop the PM2 `tamtam` server before building, build, run `pnpm db:migrate` (so a renamed/added settings key lands before the new code reads it), restart, smoke-probe critical pages, then unpause. If the smoke probe fails after restart, the script removes `.next/`, rebuilds cleanly, restarts again, and only unpauses after the probe passes. On build, migrate, restart, or smoke-recovery failure after jobs were paused, jobs remain paused for manual recovery. **Canonical post-edit command.**
