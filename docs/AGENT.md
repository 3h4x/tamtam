...
Built-in `issue-cruncher` agents default to `curl -fsS "http://localhost:1337/api/projects/by-project/<project>/issues?pick_top=1"` when the prerequisite is missing. The endpoint picks one issue server-side (trusted authors only, blocker labels and assigned issues excluded, ranked by priority labels then `updatedAt` desc), fetches its body + comments via `gh issue view`, then drops every comment whose author is not in the trust allowlist. The response shape is `{ chosenIssue: number|null, issue: { number, title, author, state, labels, url, body, comments[], droppedCommentCount } | null, reason: string|null, cached, cachedAt }` — when the eligible list is empty the endpoint returns `reason: "no_eligible_issue"` and the agent prints `NO_ELIGIBLE_ISSUE` and stops. The skill forbids the agent from running `gh issue view/list/read` or `gh api repos/*/issues/*` because TamTam has already gathered all authorized content, and issue-cruncher runs also pass Claude `--disallowed-tools` rules for `gh issue:*` plus the issue-reading `gh api` paths. Issue write actions go through TamTam's `issue-comment`, `issue-close`, and `issue-label` API routes so repo resolution and cache invalidation stay server-side. Detail responses are cached for 5 minutes in `gh_issue_detail_cache`. During validation, the shipped prompt closes stale, unverifiable, or no-longer-actionable issues as `not planned` by default; it reserves `needs-info` for recently active authors when one specific missing detail would unblock implementation. New DB-backed issue-cruncher agents get that command on creation, existing DB-backed ones are backfilled on startup, and the run path still applies the same fallback defensively for older legacy rows that truly lack a stored prerequisite. An explicit clear (`null` / empty string via the agent API) stays cleared and suppresses the default. The "open PR linked to issue" filter is deliberately deferred — `gh issue list` does not expose linked PRs natively and the existing validation step in the skill catches this case before branching.
