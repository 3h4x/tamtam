...
| `auto_fix_ci_on_red_default_branch` | boolean | `false` | Wires the missing **automatic** post-merge CI self-heal. When `true`, the periodic project sweep (`decideSweepAction`) auto-dispatches a bounded `fix-ci` for a project sitting **on its default branch, clean, with red default-branch CI** — the failure GitHub Actions surfaces after a merge. That fix-ci → `release-after-fix-ci` → release → merge loop already exists; this flag adds the trigger. Additionally gated **per-project** by `auto_push_enabled` (only self-heals default branches the operator authorized for direct pushes) and **bounded** per failing run via `lib/jobs/auto-fix-ci-state.ts` (one attempt per failing-run URL; `TAMTAM_AUTO_FIX_CI_MAX_ATTEMPTS`, default 3, consecutive distinct-failure attempts before it stops and falls back to the `ci_red` inbox HITL — merge-or-HITL preserved). Off leaves the prior behavior (red default branch is skipped by the sweep and surfaces only as a manual `ci_red` inbox button) |
