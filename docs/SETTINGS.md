...
| `auto_fix_ci_on_red_default_branch` | boolean | `true` | The **automatic** post-merge CI self-heal. When `true` (default), TamTam auto-dispatches a bounded `fix-ci` to repair a red default branch via **two triggers**: (1) the **post-merge soak phase** — after a `pr-wait` merge, soak polls the merge commit's default-branch CI and, on failure, dispatches fix-ci (the timing-gap-proof path — it catches the failure whenever GitHub surfaces it, even for a fast-cycling repo that has already moved off `main`; see `docs/PIPELINE.md`); and (2) the periodic **project sweep** (`decideSweepAction`) — for a project idle **on its default branch, clean, with red default-branch CI**. Both share the `dispatchAutoFixCiForRedDefaultBranch` helper; that fix-ci → `release-after-fix-ci` → release loop already exists — this flag adds the triggers. Gated **per-project** by `auto_push_enabled` (the sweep trigger only; only self-heals default branches the operator authorized for direct pushes) and **bounded** per failing run via `lib/jobs/auto-fix-ci-state.ts` (one attempt per failing-run URL; `TAMTAM_AUTO_FIX_CI_MAX_ATTEMPTS`, default 3, consecutive distinct-failure attempts before it stops and falls back to the revert path / `ci_red` inbox HITL — merge-or-HITL preserved). Merges are **not** held while CI is red: work still merges, and this self-heal repairs the default branch afterward. The flag is read at every gate site via `isAutoFixCiOnRedDefaultBranchEnabled()` (a **direct DB read**, not cached `getSettings()`) because the orchestrator/soak/sweep run in workflow/cron module realms where `getSettings()` returns DEFAULTS. The sweep trigger additionally requires `project_sweep_enabled`; the soak trigger runs as part of any release. Set `false` to restore the prior behavior (red default branch is skipped and surfaces only as a manual `ci_red` inbox button) |
| `ci_gate_block_dispatch_on_red` | boolean | `false` | The **opt-in "don't spin new agents while CI is red" gate.** When `true`, TamTam **defers** new **scheduled-agent** fires and **initiative** dispatch for a project whose **default-branch CI is red**, until CI goes green again. "Red" = **any** non-noise default-branch workflow with a failing conclusion (a red `Deploy` blocks too — latest-run-per-workflow via `summarizeDefaultBranchCi`, dependabot/label filtered). Deliberately **narrow**: it gates only the two paths that spin *new speculative work* — it does **not** gate **releases** (committed work still ships) and it **never** routes the sweep's `fix-ci` self-heal through the gate, so enabling it can **never** deadlock the very mechanism (`fix-ci` → `release-after-fix-ci` → green) that repairs CI. **Manual operator UI runs are unaffected.** A blocked project is never a silent stop — the existing `ci_red` inbox HITL surfaces it (merge-or-HITL preserved), and dispatch resumes **automatically** the moment CI flips green (each pass re-reads live CI). Read realm-safe via `isCiDispatchGateEnabled()` (a **direct DB read**, not cached `getSettings()`) because the cron/orchestrator realms return DEFAULTS. **Fails open** on a `gh` error (a transient hiccup never freezes the fleet). Wired at `prereqSkipReason` (scheduled-agent cron, `instrumentation-node.ts`) and `dispatchTopInitiative` (`lib/orchestrator/initiative-dispatch.ts`); see `lib/jobs/ci-dispatch-gate.ts`. Default `false` = no behavior change until enabled |
| `fix_ci_bypass_sandbox` | boolean | `true` | Scoped permission-mode escalation for the `fix-ci` job. When `true` (default), the `fix-ci` route launches the CLI in `bypassPermissions` mode (`getPermissionModeFlag('bypassPermissions')`) instead of inheriting the global `permission_mode`. **Why:** fix-ci must reproduce the CI failure locally — `pnpm install`, build, run tests — to *verify* its fix, but the Codex sandbox (`--sandbox workspace-write`) blocks all outbound network, so installs fail with `ENOTFOUND` and the agent edits blind, shipping unverified changes that can loop under `auto_fix_ci_on_red_default_branch`. Empirically, on macOS even `sandbox_workspace_write.network_access=true` does not restore connectivity under Codex's seatbelt — dropping the sandbox (`--dangerously-bypass-approvals-and-sandbox`) is the only mode that grants network. The escalation is **scoped to fix-ci only**; every other job kind keeps the global `permission_mode`. Note: when `tamtam_network_policy_strict` is on, the outer loopback seatbelt (`wrapForSandbox`, keyed off the *global* `permission_mode`) still applies by design and fix-ci must rely on pre-installed deps. Set `false` to keep fix-ci sandboxed at the cost of local verification |
| `resolve_conflicts_bypass_sandbox` | boolean | `true` | Scoped permission-mode escalation for the operator-initiated `resolve-conflicts` agent job. When `true` (default), the `resolve-conflicts` route launches the CLI in `bypassPermissions` mode. **Why:** the agent must `git rebase --continue`/`--abort` to complete the rebase, but under Codex's `auto` sandbox (`--sandbox workspace-write`) the seatbelt blocks writes to `.git/` internals (`.git/index.lock`, `.git/rebase-merge/*`) with "Operation not permitted", so the rebase stalls mid-flight and the agent reports FAILED. Dropping the sandbox restores `.git/` write access (same lesson as `fix_ci_bypass_sandbox`, but for git rebase state rather than network). Scoped to `resolve-conflicts` only; the authoritative force-push stays in server-side code (`finalizeResolveConflicts`). Set `false` to keep the agent sandboxed — fresh conflicts then can't be auto-resolved and fall back to the conflict HITL |
...
| `auto_pause_unfruitful_enabled` | boolean | `true` | When enabled, scheduled producer agents that repeatedly produce no value and either report nothing to do or finish cleanly with exit code 0 can be automatically disabled while the project and healthy sibling agents keep running. |
| `auto_pause_unfruitful_runs` | number | `6` | Consecutive no-diff scheduled runs required before auto-disabling a producer agent, with at least one clean or explicit nothing-to-do run in the window. `0` disables after the first qualifying run. |
| `auto_pause_unfruitful_rate` | number | `0.2` | Fruitful-rate floor (0–1) for the rate-based auto-disable trigger. A producer agent whose recent scheduled runs produce value in less than this fraction — over a wider sample (`max(auto_pause_unfruitful_runs × 2, 10)` runs, with ≥1 clean run) — is disabled even without an unbroken all-no-diff window. Externally gated issue/PR agents, system agents, and non-producer roles are exempt. `0` disables the rate trigger, leaving only the caught-up path. |
