...
| `auto_pause_unfruitful_enabled` | boolean | `true` | When enabled, scheduled agent runs that repeatedly produce no diff and report nothing to do can automatically pause the project until it is resumed from Settings. |
| `auto_pause_unfruitful_runs` | number | `6` | Consecutive caught-up no-diff scheduled runs required before auto-pausing a project. `0` pauses after the first qualifying run. |
