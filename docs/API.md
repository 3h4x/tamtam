...
- `/api/stats/pipeline` — Pipeline health metrics: verdict distribution, recovery-loop stats (`fix` and `fix-push`, attributed by `releaseId` when present and otherwise by the enclosing release window), step durations for `release`, `test`, `review`, `fix`, `commit`, `push`, `pr-wait`, `fix-push`, and `mark-dod` (each with `avg`, `median`, `p95`, `count`), successful-release duration stats (`mttr.avg`, `mttr.median`, `mttr.p95`, `mttr.count`), per-project breakdown, and active recovery-budget config snapshot (`maxStepIterations`, `maxFixPushAttempts`, `stepWindowSeconds`; sourced from the same helper as runtime enforcement) (GET, `?window=...`, `?project=`; 60s cache)
