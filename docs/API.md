...
- `issues` — GitHub PRs and issues (GET, with `?refresh=1` to bypass cache). `?trusted_only=1` filters the returned `issues` array server-side to authors trusted by the union of global `trusted_github_users` and per-project `.tamtam/config.yml` `security.safe_users`; the `prs` array is left untouched. `?slim=1` strips each issue to `{number, title, labels, url}` and each PR to `{number, title, labels, url, branch, isDraft}`, reducing payload size for agent prompt injection. POST merges or approves a PR and switches working copy to default after merge
