...
- `/api/agents/[agentId]` — Agent detail (GET, PATCH, DELETE). PATCH accepts optional `docPaths`, `provider`, and `prerequisiteCommand`; `null` or an empty string clears `provider`/`prerequisiteCommand`.
...
- `/api/agents/by-name` — Update agent by project+name without knowing its UUID (PATCH: `{ project, name, ...fields }`) — mirrors `/api/agents/[agentId]` writable fields, including `docPaths`, `provider`, and `prerequisiteCommand`, and enables agents to self-improve
