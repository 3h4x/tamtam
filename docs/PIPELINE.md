...
12. **Review-exhaustion fallback**: If a **NEEDS ATTENTION** review→fix loop hits `review_fix_max_iterations`, repeats the same findings (`reviewIsStuck`), or the fixer claims a Finding ID was fixed but the reviewer still flags it (`fixContradictsReview`): file a `chore(review): <count> unresolved findings from release …` issue containing the structured unresolved findings (or a quoted prose excerpt when the review did not emit structured Finding blocks), try to apply the canonical labels `tamtam` `review-followup` `priority-medium`, and skip any of those labels that do not exist in the repo, then chain to commit + push so the partial work ships. Falls back to the legacy abort if `gh issue create` fails. **DO NOT SHIP** reviews never use this path — they still stop the release before commit/push.
