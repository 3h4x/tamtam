...
| **PR-branch execution gate** (`lib/security/pr-branch-execution.ts`) | Refuses host-side project-code execution on non-default branches unless the current branch is known and every commit the branch adds over the trusted base resolves through GitHub to an `author.login` in `safe_users` / `trusted_github_users` (or the branch adds no commits at all), or the caller explicitly approves the supported test-run override; uncommitted working-tree changes do not gate execution; also blocks auto-merge when the GitHub PR diff touches dependency manifests, build scripts, workflow files, Dockerfiles, Makefiles, or JS/TS config files |
...
| **Merge/approve author-trust gate** (`app/api/…/issues` POST, `lib/github/pr-author.ts`) | Resolves the PR author via `gh` and refuses `merge`/`approve` with `403` unless the author is in `safe_users` / `trusted_github_users` (fail closed on lookup failure). On a public repo anyone can open a PR; green CI plus a decoupled project-level LGTM must not become a one-click merge of untrusted code onto the default branch. The inbox `pr_ready_to_merge` signal is gated the same way (an untrusted-author PR is never surfaced as ready-to-merge), and the Issues tab already filters untrusted-author PRs from display — three consistent layers. Pipeline auto-merge is unaffected (it acts only on PRs TamTam itself opened via the operator's `gh` identity). |
...
Project test commands, review prerequisite commands, and dev-server start commands execute project code directly, so they use a stricter environment than ordinary CLI model runs. These host-side commands on non-default branches are gated by `checkPrBranchExecutionGate`: TamTam requires a known current branch, lists the commits the branch adds over the trusted base (`origin/<default>..HEAD`), resolves each commit through GitHub, and trusts only the returned `author.login` value. Local git author names and emails are ignored because they are user-controlled. If branch detection fails, the branch's commit list cannot be read, the GitHub repository cannot be resolved, a commit that GitHub *has* resolves to no mapped author, or any commit maps to a login outside `safe_users` / `trusted_github_users`, TamTam refuses to run the host command. A commit GitHub has never seen (never pushed — GitHub answers `GET …/commits/{sha}` with HTTP 422 "No commit found for SHA") is **trusted local work**: external contributions always arrive as commits already present on GitHub (a PR head, which the same API resolves to the contributor's login), so a commit that exists only in the local clone can only be the operator's or TamTam's own agent output. This case matters because the gate runs at the pipeline's *first* step (test), before the push phase, so freshly-made local commits are routinely not yet on GitHub; failing them closed wrongly blocked every such release. The "absent from GitHub" signal is matched narrowly on that exact missing-commit message — a network / auth / rate-limit failure is indeterminate and still fails closed, so untrusted code cannot run merely because GitHub was briefly unreachable. A branch that adds **no** commits over the base is trusted (its committed content equals the already-trusted base — nothing external to verify). The test API accepts `approveUntrustedPrBranch: true` for explicit operator approval (skips the gate entirely).

The gate deliberately does **not** require a clean working tree. The one external-code vector it defends against is a branch whose *committed* history was pulled in by checking out someone's PR head — those commits execute during `pnpm test` et al., which is why every commit is author-verified. Uncommitted working-tree changes have a different provenance: nothing external can write them (an external contribution arrives as commits, verified above), so they can only be the operator's or TamTam's own agent output — the *same* trust the default branch is granted unconditionally. Gating on a dirty tree therefore added no protection against the documented threat; it only produced false positives that blocked legitimate local work (e.g. a sweep-triggered auto-release on a trusted, already-merged feature branch that still carried uncommitted edits, or globally-ignored editor junk surfacing as untracked when `HOME` was unset). `HOME` is still pinned into the gate's git/gh subprocess env (`GIT_OPTS.env`, from `os.homedir()`; `scripts/pm2-start.sh` / `ecosystem.config.js` also pin it on the server process, which PM2 forks from a stripped env with no `HOME`) — git needs it to read repo/user config for `git log`, and gh needs it for its auth/hosts config when resolving commit authors.

Releases still record a `trustedLocalChanges` provenance marker on the release meta-job (`start-release.ts`) when they originate from **TamTam's own in-process agent run** (issue-cruncher, or a manual issue-linked `run`) or an **operator-initiated release** (`operatorInitiated: true`, the UI Release button). This is now a trace/audit signal only — the execution gate no longer consults it, because a dirty working tree never gates execution. Test, review-prerequisite, and dev-server child processes are spawned with credential-like environment variables stripped (`*_TOKEN`, `*_SECRET`, `*_PASSWORD`, API keys, AWS/GCP/Google credentials, npm auth token variables). PR auto-merge is additionally blocked when the actual GitHub PR diff changes high-risk execution files such as dependency manifests, package-manager lockfiles, `.github/workflows/*`, `Makefile`, `Dockerfile`, build configs, or JS/TS config files; failure to inspect the PR diff is treated as risky and blocks auto-merge.

### Operator-initiated conflict resolution

The `resolve-conflicts` route (`POST /api/projects/by-project/[name]/resolve-conflicts`, wired to the inbox "Resolve conflicts" HITL) is a **new, narrowly-scoped autonomous git-write boundary**: TamTam rebases an open PR branch onto its base and resolves the conflict hunks with an agent, then force-pushes the result. This is the one thing the background stranded-branch sweep deliberately refuses to do unattended (`lib/jobs/pr-behind-rebase.ts` aborts a conflicting rebase and hands off to a HITL). It is acceptable here **only because it is operator-initiated** — the HITL click is the explicit request that the "Destructive Operations" rule (no force-pushes / history rewrites without explicit request) requires. It is never fired from the 30s sweep.

Responsibility is split so the dangerous operation stays in deterministic code:

- **Route (setup + gate):** resolves the PR via `gh`, refuses unless it is an OPEN, still-`CONFLICTING` PR, `git fetch`es the base, checks out the branch (refusing a dirty worktree so no local work is clobbered), and runs `checkPrBranchExecutionGate` — the same author-trust boundary that guards test/review execution — so an agent never edits code on a branch whose added commits map to an untrusted `author.login`. The base branch is trusted unconditionally, so the rebase always targets the **freshly-fetched** `origin/<base>`.
- **Agent (resolution only):** rebases locally onto the pre-fetched `origin/<base>` and resolves the hunks to a clean tree; it **must not push** (the prompt forbids it and finalize owns the push). It runs with `bypassPermissions` gated on the `resolve_conflicts_bypass_sandbox` setting (default on) — under codex's `auto` sandbox the workspace-write seatbelt blocks writes to `.git/` internals (`.git/index.lock`, `.git/rebase-merge/*`), so `git rebase --continue`/`--abort` fail with "Operation not permitted" and the rebase cannot complete; dropping the sandbox restores `.git/` write access (the same escalation fix-ci uses for network). The conflict hunks embed the PR diff, so they are wrapped as untrusted content (`withUntrustedPreamble` / `wrapUntrusted`), and the author-trust gate above is the real boundary against a hijacked branch.
- **`finalizeResolveConflicts` (verify + push):** on job completion, trusts git/gh state — not the agent's word — and only force-pushes when it independently confirms a clean, marker-free tree that is `behind==0` / `ahead>0` on the freshly-fetched base and still an OPEN PR for the exact branch. The push is always `git push --force-with-lease` (never bare `--force`; the lease protects commits pushed to the branch since the fetch). It then hands to `pr-wait`, so CI re-validates the model-authored resolution before any merge — never a blind merge.

Termination preserves the **merge-or-HITL** invariant: success → force-push + `pr-wait` (merges when green, or re-raises a HITL); any failure (unverified tree, still-behind, markers remaining, lease rejection, closed PR) → `git rebase --abort` to a clean worktree + re-raise the conflict HITL. The whole flow is operator-triggered and dup-guarded, so a permanently-unresolvable conflict cannot loop — it just returns to the inbox for the operator to decide again.
