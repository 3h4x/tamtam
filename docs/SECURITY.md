...
The "uncommitted or untracked changes" check runs `git status --porcelain --untracked-files=all`, which only excludes globally-ignored files (editor junk, `.DS_Store`, `.playwright-mcp/` snapshots) when git can read the operator's global `core.excludesFile` — i.e. when `HOME` is set. The gate injects `HOME` into its git/gh subprocess env (`GIT_OPTS.env`, from `os.homedir()`), and `scripts/pm2-start.sh` / `ecosystem.config.js` pin `HOME` on the server process itself, because PM2 forks the app from a stripped env with no `HOME`. Without that, git ignores the global gitignore and globally-ignored files surface as untracked, so the gate would **refuse a release on a tree the operator's own `git status` reports clean** — a silent false positive.
