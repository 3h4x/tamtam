...
- **PM2 zombie next-server.** If `pm2 restart tamtam` leaves an orphan `next-server` listening on 1337, the new PM2 process errors with EADDRINUSE and the orphan keeps serving stale code. Symptoms: edits don't show up; both ports busy. Fix: `lsof -ti:1337 | xargs kill -9; pm2 delete tamtam; pnpm dev`.
