...
describe.concurrent('scripts/qa-mocks/gh', () => {
  let dir: string;
  ...
  describe.concurrent('repo view', () => {
    it('returns JSON with nameWithOwner when no filter flags', async () => {
    ...
    it('returns just the owner/repo string with --jq flag', async () => {
      const result = await runGh(['repo', 'view', '--jq', '.nameWithOwner']);
      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe('qa/mock-repo');
    });

    it('returns just the owner/repo string with -q flag', async () => {
      const result = await runGh(['repo', 'view', '-q', '.nameWithOwner']);
      expect(result.stdout.trim()).toBe('qa/mock-repo');
    });
  });

  describe.concurrent('pr commands', () => {
    it('pr list returns array with expected PR shape', async () => {
    ...
    it('pr view returns PR details with mergeStateStatus', async () => {
  ...
  describe.concurrent('issue commands', () => {
    it('issue list returns array with tamtam-labelled issues', async () => {
    ...
    it('issue view returns issue details', async () => {
  ...
  describe.concurrent('label and run commands', () => {
    ...
    it('run view returns mocked log output', async () => {
  ...
  it('returns empty JSON object for unknown subcommands', async () => {
