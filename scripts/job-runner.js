...
async function isJobsPaused() {
  try {
    const dbUrl = process.env.DATABASE_URL;
    if (!dbUrl) return false;
    // Loaded lazily so the runner still fail-opens if pg is not installed
    // in a test or recovery environment.
    const { Client } = require('pg');
    const client = new Client({ connectionString: dbUrl, connectionTimeoutMillis: 3000 });
    await client.connect();
    try {
      const r = await client.query("SELECT value FROM settings WHERE key = 'jobs_paused'");
