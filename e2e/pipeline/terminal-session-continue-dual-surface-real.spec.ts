...
const PROJECT = 'terminal-run-session-continue-dual-surface';
const INITIAL_PROMPT =
  'Finish the first ordinary run so the canonical session route can be resumed later.';

test.describe('Real resumed ordinary run lifecycle across history and terminal session route', () => {
  test('history and the canonical terminal session route both pick up a continued run, then clear live state after success', async ({
    page,
    request,
  }) => {
    ...
    writeScenario(PROJECT, [
      {
        label: 'run',
        sleep_ms: 2000,
        text: 'Initial session output is available before the continuation starts.',
      },
    ]);
    ...
    writeScenario(PROJECT, [
      {
        label: 'run',
        sleep_ms: 10000,
        text: 'Resumed ordinary run output arrived on the existing session route.',
      },
    ]);
    ...
    await expect(
      terminalPage.getByText('Initial session output is available before the continuation starts.'),
    ...
    expect(continuedJob?.['exit_code'], 'continued run exit code').toBe(0);
    ...
    await expect(
      terminalPage.getByText('Resumed ordinary run output arrived on the existing session route.'),
    ).toBeVisible({ timeout: 20_000 });
    await expect(terminalPage).toHaveURL(
