...
test.describe('Real workflow-runs failure and cancellation transitions', () => {
  test('failed release leaves the active panel and appears in attention with its project and error text', async ({
    ...
    const failedRun = await waitForWorkflowRunStatus(request, FAILURE_PROJECT, 'failed');
    ...
    if (failedRun.error) {
      await expect(failureRow.getByText(failedRun.error.split('\n')[0].slice(0, 60))).toBeVisible();
    }
    await expect(workflowRunLink(activePanel, FAILURE_PROJECT)).toHaveCount(0, { timeout: 15_000 });
    await expect(page.getByRole('button', { name: /failed \d+/i })).toBeVisible();
  });

  test('cancelled release leaves the active panel and appears in attention with its project and cancelled badge', async ({
    page,
    request,
  }) => {
    ...
    await expect(page.getByRole('button', { name: /cancelled \d+/i })).toBeVisible();
  });
});
