...
// A release that exited non-zero with NO children — the "blocked" case.
// The component derives outcomeStatus = 'blocked' when exitCode !== 0 and
// children.length === 0, which renders "Retry release" instead of "Continue release".
function blockedReleaseJobs(): MockJob[] {
  const releaseStarted = now() - 60
  return [
    {
      id: RELEASE_ID,
      project: PROJECT,
...
test.describe('History retry release (blocked)', () => {
  test('blocked release (no children) shows "Retry release" button and posts the correct body on click', async ({
    page,
  }) => {
    await stubHistoryShell(page, blockedReleaseJobs)
    ...
    await page.route(`**/api/projects/by-project/${PROJECT}/release`, async (route: Route) => {
      ...
      await route.fulfill({ json: { status: 'started', job_id: 'history-retry-blocked-new-release' } })
    })
    ...
    // Blocked release: button label must be "Retry release", not "Continue release"
    const retryBtn = releaseRow.getByRole('button', { name: 'Retry release' })
    await expect(retryBtn).toBeVisible({ timeout: 8_000 })
    await expect(releaseRow.getByRole('button', { name: 'Continue release' })).toHaveCount(0)
    ...
    // Button re-enables after the POST resolves
    await expect(retryBtn).toBeEnabled({ timeout: 8_000 })
  })

  test('blocked release button is disabled when jobs are paused', async ({ page }) => {
    await stubHistoryShell(page, blockedReleaseJobs)

    // Override settings to report jobs paused
    await page.route('**/api/settings', (route: Route) =>
