...
const PROJECT = 'workflow-runs-terminal-late-start-mocked';
const RELEASE_ID = 'workflow-runs-terminal-late-start-mocked-release';
const SUCCESS_OUTPUT = 'Late-start mocked release finished successfully after both surfaces were idle.';
const FAILURE_DETAIL = 'Mocked push failed after the release appeared late on both surfaces.';
const CANCELLED_DETAIL = 'Release was cancelled after the terminal landing page initially stayed idle.';
...
function runningReleaseJob() {
  return {
    ...
    context_meta: null,
    provider: 'claude',
    work_summary: 'Late-start mocked release is now running.',
...
function workflowRun(
  ...
  if (status === 'running') {
    return {
      ...
      startedAt: '2026-06-27T09:00:05.000Z',
      completedAt: null,
      durationMs: null,
      input: [PROJECT, { triggeredBy: 'agent-mocked-late-start' }],
      output: null,
      error: null,
    };
  }
  ...
  if (status === 'failed') {
    return {
      ...
      createdAt: '2026-06-27T09:00:00.000Z',
      startedAt: '2026-06-27T09:00:05.000Z',
      completedAt: '2026-06-27T09:00:20.000Z',
      durationMs: 15_000,
      input: [PROJECT, { triggeredBy: 'agent-mocked-late-start' }],
      output: null,
...
async function stubSharedRoutes(
    ...
    (route: Route) => {
      ...
      const jobs =
        phase === 'running'
          ? [runningReleaseJob()]
          : phase === 'failed'
            ? [finishedReleaseJob(2, 'Late-start mocked release failed on both surfaces.\n', FAILURE_DETAIL)]
            : phase === 'cancelled'
              ? [finishedReleaseJob(-3, 'Late-start mocked release was cancelled on both surfaces.\n')]
              : phase === 'success'
  ...
  await context.route(
    ...
    (route: Route) => {
      ...
      const done = phase === 'idle' || phase === 'running' ? 0 : 1;
      const failed = phase === 'failed' ? 1 : 0;
      const aborted = phase === 'cancelled' ? 1 : 0;
      route.fulfill({
        json: {
...
test.describe('Mocked workflow-runs late-start dual-surface release lifecycle', () => {
  test('both surfaces pick up a late-start mocked release and settle to success without reload', async ({
    page,
  }) => {
    let phase: Phase = 'idle';
    let releaseJobPolls = 0;
    ...
    await expect.poll(() => releaseJobPolls, { timeout: 6_000 }).toBeGreaterThan(0);
    ...
    await expect(page.getByText('No workflow runs yet')).toHaveCount(0, { timeout: 12_000 });
  ...
  test('both surfaces pick up a late-start mocked release and settle to failure without reload', async ({
    page,
  }) => {
    let phase: Phase = 'idle';
    let releaseJobPolls = 0;
    ...
    await page.context().route(`**/api/streaming/${RELEASE_ID}`, async (route: Route) => {
      ...
      await route.fulfill({
        ...
        headers: { 'content-type': 'text/event-stream', 'cache-control': 'no-cache' },
        body: [
          'data: Late-start mocked release failed on both surfaces.',
          '',
          'event: done',
    ...
    await expect(terminalPage.getByText('Late-start mocked release failed on both surfaces.')).toBeVisible({
      timeout: 12_000,
    });
  ...
  test('both surfaces pick up a late-start mocked release and settle to cancelled without reload', async ({
    page,
  }) => {
    let phase: Phase = 'idle';
    let releaseJobPolls = 0;
    ...
    await page.context().route(`**/api/streaming/${RELEASE_ID}`, async (route: Route) => {
      ...
      await route.fulfill({
        ...
        headers: { 'content-type': 'text/event-stream', 'cache-control': 'no-cache' },
        body: [
          'data: Late-start mocked release was cancelled on both surfaces.',
          '',
          'event: done',
    ...
    await expect(terminalPage.getByText('Late-start mocked release was cancelled on both surfaces.')).toBeVisible({
      timeout: 12_000,
    });
