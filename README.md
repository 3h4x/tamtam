...
| **Release pipeline** | Quality-gated, branch-context-driven flow: test → review → fix loop → commit → push → DoD (`mark-dod`) → pr-wait/merge → soak. Default-branch releases push directly; non-default branches open or reuse a PR. Review-side fix verification follows `review_fix_max_iterations` (default `0`, unlimited until LGTM or release timeout); test/commit verification and non-hook push verification default to 3 attempts, while push-hook rejection fixes are capped at 2 attempts |
