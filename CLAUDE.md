...
- `pnpm mcp:http <tool> [json_args]` — call local TamTam HTTP endpoints through the sibling `mcp-http-tools` checkout using `.tamtam/mcp-http-tools.yaml` (set `MCP_HTTP_TOOLS_DIR` if the checkout is not at `../mcp-http-tools`).
...
- **Local HTTP MCP tools**: `.tamtam/mcp-http-tools.yaml` defines read-only TamTam API tools for the sibling `/Users/3h4x/workspace/mcp-http-tools` project. Use `pnpm mcp:http tamtam_health` or `pnpm mcp:http tamtam_project_recommendations '{"project":"tamtam"}'` for repeatable local API checks without ad hoc curl commands.
