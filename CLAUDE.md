...
- Background probe sweep: `instrumentation-node.ts` schedules `runProbeSweep` every 30s — detects providers that hang after the final result event and resolves them via `probeJobStatus`.
