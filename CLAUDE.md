...
The `rebuild` script is now graceful by default (`scripts/rebuild-safe.sh`): it pauses jobs via `PATCH /api/settings {jobs_paused:true,rebuild_in_progress:true}`, polls `/api/jobs?status=running&limit=200&offset=...` until blocking pipeline-step/fix-ci/agent/run jobs drain (default 10 min, override via `TAMTAM_REBUILD_DRAIN_TIMEOUT`), then builds, runs `pnpm db:migrate` so a renamed/added settings key lands before the new code reads it (a failed migrate aborts before restart), restarts via `pm2-start.sh`, smoke-probes `/`, `/workflow-runs`, `/agents`, and `/settings/general`, and unpauses. If the smoke probe fails after restart, it does a clean `.next/` rebuild/restart before unpausing. `pr-wait` is excluded from the drain set because its on-boot resume handles mid-poll interruption. If the build fails the pause is reverted; if the restart fails the pause is *kept* on so the half-restarted server doesn't pick up new work — clear it manually via `/settings`. Bare `pnpm rebuild` still runs pnpm's built-in native-deps rebuild; for the legacy "kill everything immediately" behavior, use `pnpm run rebuild:force` (equivalent to the old `pnpm build && pnpm start`).
...
- **Singletons on `globalThis`** are intentional in a few places: `__tamtamCronWorker`, `__tamtamJobCancellation`, `__tamtamStartingAgents`, `__tamtamSpawnedClosePending`, `__tamtamAgentLastSkip`, `__tamtamProjectRecoveryDrains`, `__tamtamWorkflowRunsPool`, `__tamtamBrowserBroker`, `__tamtamBrowserBrokerStarting`, `__tamtamAgentLastDispatch`, `__tamtamOrchestratorHistory`, `__tamtamQuotaPersistedAt`, `__tamtamQuota`, `__tamtamCodexQuota`, `__tamtamJobsCache`, `__tamtamJobsCacheLoaded`, `__tamtamAllFileAgentsCache`, `__tamtamFileAgentOverrideCache`, `__tamtamFileAgentOverridePending`, `__tamtamTestScheduler`, `__tamtamStartingPipelineSteps` (atomic per-(release,phase) start claim — `lib/pipeline/pipeline-start-slot.ts`, prevents duplicate phase dispatch). Only add another when cross-route coordination truly requires it; pin to `globalThis` because Next.js duplicates modules; document in the relevant `docs/*.md`.
