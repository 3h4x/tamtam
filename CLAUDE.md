...
- **Agents** — composed from skills + project docs + model + prompt + optional interval schedule + runner; agents can also pin a provider and prerequisite command. The intake path runs as a Vercel Workflow (`lib/agents/intake-workflow.ts`); historical `runner: pm2 | launchctl` frontmatter on file agents is read for back-compat but the CLI lifecycle is owned by the workflow runtime regardless. `launchctl` is deprecated.
...
- `scripts/` — server startup + CLI shims (`pm2-start.sh`, `claude-shim.js`, `gemini-shim.js`, `lmstudio-shim.js`, `codex-shim.js`, `shim-utils.js`, `redact-log-stream.js`). Jobs are spawned in-process from Workflow steps now; there's no separate runner script.
...
- `/workflow-runs` — Vercel Workflow runs (list with name/status/duration filters); `/workflow-runs/[runId]` — run + steps detail
- `/logs` — Log viewer
...
- Background probe sweep: `instrumentation-node.ts` schedules `runProbeSweep` every 30s (and once at boot). It detects Claude-CLI processes that hung after emitting their final result event and resolves them via `probeJobStatus`, and aborts any release that crosses its `releaseDeadlineAt` wall-clock deadline. The hook-failure recovery tickers that used to run alongside it (queued-agent drain, generic reconcile, auto-resume of stuck releases, quota drain) were removed when workflow-driven releases became the only path — the workflow runtime owns those concerns. `drainBootRecoveryWork` still fires once at boot for cross-restart cleanup.
...
- **One-shot job processes**: CLI jobs are spawned **in-process** from inside the Next.js server. The agent intake path runs as a Vercel Workflow (`lib/agents/intake-workflow.ts` with `'use workflow'` / `'use step'`); its spawn step calls `lib/jobs/inline-agent.ts startInProcessAgentJob`, which awaits the child's exit and then calls `markDone()` → completion hooks. Pipeline steps (review, fix, pr-review) use a fire-and-forget detached helper at `lib/jobs/spawn-claude-detached.ts startJobInProcess` — same call shape as the retired `startJob`, but `child_process.spawn` + `on('close')` instead of PM2. Inline orchestrator jobs (`mark-dod`, `pr-wait`) run in the Next.js process and are reaped on boot if a restart abandons them. The `${jobId}.prompt` file is still written so `/api/jobs/[jobId]/rerun` can restore the original prompt. **PM2 only supervises the `tamtam` Next.js server itself** — no per-job PM2 entries are created.
- **Release pipeline orchestrator**: under `lib/workflows/`, a Vercel Workflow state machine drives every release (test → review → fix loop → commit → push → dod → merge). The orchestrator (`release-orchestrator.ts`) waits for the just-finished sub-step, asks `decideNextPhase` for the next decision, runs `applyReleaseGuards` over it (convergence + iteration caps), then dispatches the matching `release*PhaseWorkflow` via `dispatch-phase.ts`. The 7 phase workflows (test/review/fix/commit/push/mark-dod/pr-wait) live under `lib/workflows/phases/`. Each phase spawn step wraps `startProject*` in `runWithParent(releaseJobId, ...)` so spawned `test`/`review`/`commit`/`push`/`fix` rows inherit `release_id` correctly via `parentContext` AsyncLocalStorage — without this, completion hooks lose the workflow-driven short-circuit and double-dispatch alongside the orchestrator. The legacy completion-hook chain in `lib/jobs/lifecycle.ts` short-circuits on `releaseId` for any release-linked pipeline step (the orchestrator owns chaining); standalone (no-`releaseId`) jobs still flow through the chain blocks because they have no orchestrator. The 7 fix scenarios (test fail / review NEEDS ATTENTION / push hook rejection / commit failure) all spawn the **single** generic `fix` job kind via `start-fix.ts` — the former `fix-push` kind was unified in. Full reference: `docs/PIPELINE.md`.
- **Release-pipeline guardrails** (`lib/workflows/guards/`): the orchestrator owns three pre-dispatch guards that bound runaway fix loops. `reviewIsStuck` aborts when the current review re-flags the same finding fingerprint as a prior review in the release. `fixContradictsReview` aborts when the prior fix claimed `Status: fixed` for a Finding ID the current review still flags. `checkIterationCap` aborts when a `fix → re-verify` dispatch would exceed `maxStepIterations` (test/commit/push), `reviewFixMaxIterations` (review), or `getPushFixAttemptCap` (push-hook fix retry). When abort fires for a NEEDS-ATTENTION review-side guard, `finalizeReleaseStep` calls `fileReviewExhaustionIssue` to file a follow-up GitHub issue with the persistent findings. All abort decisions carry a `stopReason` that's persisted on the release meta-job's `contextMeta.releaseStopReason` so the trace UI explains the failure.
