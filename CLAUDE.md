...
- **Release pipeline orchestrator** (opt-in): under `lib/workflows/`, a Vercel Workflow state machine can drive the release pipeline (test → review → fix loop → commit → push → dod → merge). Gated by two env vars in `.env.local`: `TAMTAM_RELEASE_WORKFLOW=1` enables observability-only mode (every release gets a `workflow_runs` row, hooks still drive); add `TAMTAM_RELEASE_WORKFLOW_DRIVE=1` for full drive mode (orchestrator stamps `contextMeta.workflowDriven` and dispatches the matching `release*PhaseWorkflow` child per decision; completion hooks short-circuit). Both default off — production pipeline is unchanged. The 8 phase workflows live under `lib/workflows/phases/`. Full reference: `docs/PIPELINE.md` (Vercel Workflow orchestrator section).
