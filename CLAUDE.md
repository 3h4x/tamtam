...
**Run `pnpm run rebuild` yourself when a change needs to go live** — e.g. to visually verify a frontend edit (the `:1337` server runs the built output via `next start`, so it does NOT hot-reload source edits). Do not stop and ask the operator to rebuild; the graceful script is safe and self-recovering from an interactive/external session — it pauses jobs, drains running work, serves a placeholder, builds, migrates, restarts, smoke-probes, and unpauses on its own, and only *keeps* jobs paused if a step fails (recoverable via `/settings`). Prefer `run_in_background: true` since a rebuild can take minutes (and longer when the machine is oversubscribed). **The one hard exception** — never rebuild from inside a TamTam-spawned in-process agent job (the drain waits on that job → deadlock, and `pm2 stop tamtam` kills the server it runs under); leave that to an out-of-band session. The Codex-sandbox exception below also still applies.
...
- **Settings in workflow/cron realms:** `getSettings()` (`lib/shared/config.ts`) reads a **module-local** cache and returns `DEFAULTS` when it is unpopulated — which it is inside the workflow-runtime and graphile-worker module realms (separate module instances from the API realm that `PATCH /api/settings` warms). Code that gates behavior inside the release orchestrator, workflow phases, or cron tasks MUST NOT trust `getSettings()` for a recently-toggled flag; read it straight from the `settings` table there (pattern: `isAutoFixCiOnRedDefaultBranchEnabled()` in `lib/jobs/auto-fix-ci-state.ts`). A silent no-op of a "why won't my flag take effect in the sweep/soak/orchestrator" shape is almost always this.
...
- **CI-red dispatch gate** (`lib/jobs/ci-dispatch-gate.ts`, opt-in `ci_gate_block_dispatch_on_red`, default off): when a project's default-branch CI is red (any failing workflow, via the sweep's pure `summarizeDefaultBranchCi`), **scheduled-agent** fires (`prereqSkipReason`, `system` agents exempt) and **initiative** dispatch (`dispatchTopInitiative`) are deferred until CI goes green. Releases and the sweep's `fix-ci` self-heal are **not** gated — work still ships and CI can still be repaired, so the gate can't deadlock its own cure — and manual UI runs are unaffected. The flag is read realm-safe (direct DB, not `getSettings()`), fails open on a `gh` error, and a blocked project stays visible via the `ci_red` inbox HITL. Distinct from the auto-fix-ci self-heal (`__tamtamAutoFixCiState`): that repairs red CI, this defers new work on it.
