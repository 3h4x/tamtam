...
- **Permission mode** for headless Claude jobs defaults to `acceptEdits` (auto-accepts file Read/Edit/Write). Never use `auto` for headless — there is no UI to approve prompts, so writes can hang indefinitely.
