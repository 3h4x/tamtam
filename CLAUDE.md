...
- `/api/projects/by-project/[name]/create-pr` — Push current branch + create GitHub PR (POST); returns `{ url }` — refuses if on default branch; derives Conventional-Commits title + body from commit log, falls back to `gh pr create --fill`
